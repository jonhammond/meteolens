...
- **Power BI real-time streaming retirement** → the "New streaming dataset" UI shows a "retired soon" dialog; per Microsoft Learn (`service-real-time-streaming`, updated Dec 2025) *creation* of new streaming/push semantic models stays enabled until **2027-10-31**, and **existing models remain unaffected** after (Push URL + PostRows API keep working). Decision (Aug 2026): proceed — the free-tier alternatives don't exist (Fabric RTI needs paid capacity; import-mode Postgres needs a gateway + Windows-only Desktop). Click **OK** on the dialog and continue. If Microsoft ever forces migration, Supabase holds all history and `backfill_powerbi.py` is the escape hatch to whatever replaces push datasets.
