...
- **Blockers:** none in code — pipeline recovery waits on deploy of the API-key change + **[USER]** Render env var.
- **Next immediate step:** deploy the 429 fix (commit+push to `main`, [USER] sets `OPEN_METEO_API_KEY` on Render), verify an all-`ok` ingest via the GH Actions workflow, then restart the pipeline monitor and run the M6 2-hour-green accept.
