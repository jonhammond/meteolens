...
- [ ] **[USER]** New → Streaming dataset → API, name `meteolens`, fields per PLAN.md M7 step 1, **Historic data analysis: ON**; copy Push URL → set `POWERBI_PUSH_URL` on Render (secret — never in git). *A "this feature will be retired soon" dialog appears — click **OK** and continue; creation is supported until 2027-10-31 and existing models remain unaffected after (see PLAN.md Risks)*
